...
> **무료 버전 안내**: 이 버전에서는 특정글 분석만 가능합니다.
> 전체분석(12개 항목)은 블크업 과정에서 제공됩니다.
...
- 파일 있음 → 해당 blog_id의 **최신** summary/posts 파일 자동 선택
  ...
  - 포스트 2개 이상 → 포스트 목록 표시 후 번호 선택 대기
...
사용자가 계정 번호를 입력하면, 해당 계정의 최신 `*_summary_*.json` 파일에서 포스트 목록 표시:
